...
class ConversationManager:
    ...
    def ensure_storage_dir(self):
        ...
        try:
            ...
            logger.error(f"STORAGE: Failed to create or write to storage directory {self.storage_dir}: {e}")
            raise
    ...
    def create_conversation(self) -> str:
        ...
        try:
            ...
            logger.debug(f"STORAGE: Conversation file created successfully at: {file_path}")
    ...
    def load_conversation(self, continuation_id: str) -> Optional[Dict[str, Any]]:
        ...
        if continuation_id in self._conversation_cache:
            ...
            logger.debug(f"STORAGE: Cached conversation has {len(cached_data.get('messages', []))} messages")
            return cached_data
        ...
        try:
            ...
            self._conversation_cache[continuation_id] = conversation_data
            logger.debug(f"STORAGE: Loaded conversation {continuation_id} with {len(conversation_data.get('messages', []))} messages from file")
    ...
    def add_message(self, continuation_id: str, role: str, content: str, 
        ...
        """
        logger.debug(f"STORAGE: Adding {role} message to conversation {continuation_id}")
        
        conversation_data = self.load_conversation(continuation_id)
        if not conversation_data:
            logger.error(f"STORAGE: Cannot add message to non-existent conversation: {continuation_id}")
            return False
        ...
        logger.debug(f"STORAGE: Message added to conversation data. Total messages: {len(conversation_data['messages'])}")
        ...
        if verification_data and len(verification_data["messages"]) == len(conversation_data["messages"]):
            logger.info(f"STORAGE: Successfully added {role} message to conversation {continuation_id}. Total messages: {len(verification_data['messages'])}")
            return True
        else:
            logger.error(f"STORAGE: Failed to verify message save for conversation {continuation_id}")
            return False
    
    def get_conversation_history(self, continuation_id: str, max_tokens: Optional[int] = None) -> List[Dict[str, str]]:
        ...
        if max_tokens and len(openai_messages) > 0:
            ...
            if estimated_tokens > max_tokens:
                ...
                logger.debug(f"Optimized conversation {continuation_id}: {len(openai_messages)} -> {len(optimized_messages)} messages")
                openai_messages = optimized_messages
        
        logger.debug(f"Retrieved {len(openai_messages)} messages for conversation {continuation_id}")
        ...
        try:
            for filename in os.listdir(self.storage_dir):
                if filename.startswith("conversation_") and filename.endswith(".json"):
                    continuation_id = filename[13:-5]  # Remove "conversation_" and ".json"
    ...
    def delete_conversation(self, continuation_id: str) -> bool:
        ...
        try:
            if os.path.exists(file_path):
                ...
                logger.warning(f"Conversation not found for deletion: {continuation_id}")
                return False
        except Exception as e:
    ...
    def cleanup_old_conversations(self, max_age_days: int = 30):
        ...
        logger.info(f"Cleaned up {deleted_count} old conversations (older than {max_age_days} days)")

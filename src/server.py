...
def handle_tools_list(req_id):
    ...
    tools = [
        {
            "name": "chat",
            "description": "Chat with OpenRouter AI models. ⚠️ CRITICAL INSTRUCTIONS: 1) You MUST include ALL related files and fully understand the problem by scanning the code yourself BEFORE you send ANY query to the LLMs or you risk it not having enough background information to return an optimal and correct response. Always attach relevant files, read documentation, and provide complete context. 2) You MUST ONLY use these exact model aliases: 'gemini', 'deepseek' - NEVER use full OpenRouter model names like 'google/gemini-pro-1.5'! 3) CRITICAL: You MUST use the continuation_id from previous responses in follow-up messages to maintain conversation context - this is REQUIRED for proper conversation flow!",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "prompt": {"type": "string", "description": "Message to send - MUST include full context and background information"},
                    "model": {"type": "string", "description": "⚠️ CRITICAL: You MUST use ONLY these exact aliases - DO NOT use full OpenRouter model names! Use: 'gemini' (for google/gemini-2.5-pro), 'deepseek' (for deepseek/deepseek-r1-0528). NEVER use google/gemini-pro-1.5 or any other full model names!", "default": DEFAULT_MODEL},
